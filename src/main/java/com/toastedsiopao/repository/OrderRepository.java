...
public interface OrderRepository extends JpaRepository<Order, Long> {
	...
	@Query(value = "SELECT o.id FROM Order o "
			+ "WHERE o.user = :user AND o.status = :status ORDER BY o.orderDate DESC", countQuery = "SELECT COUNT(o) FROM Order o WHERE o.user = :user AND o.status = :status")
	Page<Long> findIdsByUserAndStatusOrderByOrderDateDesc(@Param("user") User user, @Param("status") String status,
			Pageable pageable);

	@Query(value = "SELECT o.id FROM Order o WHERE " + "(:startDateTime IS NULL OR o.orderDate >= :startDateTime) AND "
	...
	@Query(value = "SELECT o.id FROM Order o WHERE " + "(CAST(o.id AS string) LIKE CONCAT('%', :keyword, '%') OR "
			...
			+ "ORDER BY o.orderDate DESC", countQuery = "SELECT COUNT(o) FROM Order o WHERE "
					...
					+ "(:startDateTime IS NULL OR o.orderDate >= :startDateTime) AND "
					+ "(:endDateTime IS NULL OR o.orderDate <= :endDateTime) ")
	Page<Long> findIdsByKeyword(@Param("keyword") String keyword, @Param("startDateTime") LocalDateTime startDateTime,
			@Param("endDateTime") LocalDateTime endDateTime, Pageable pageable);
	...
	@Query("SELECT o FROM Order o " + "LEFT JOIN FETCH o.items oi " + "LEFT JOIN FETCH oi.product p "
			+ "LEFT JOIN FETCH o.issueReports " + "WHERE o.id IN :ids ORDER BY o.orderDate DESC")
	...
	// --- FIX: Use CAST(... AS date) for H2/MySQL compatibility instead of
	// FUNCTION('DATE', ...) ---
	@Query("SELECT CAST(o.orderDate AS date) AS orderDay, SUM(o.totalAmount) AS dailySales FROM Order o WHERE o.orderDate BETWEEN :start AND :end AND o.status = 'DELIVERED' GROUP BY orderDay ORDER BY orderDay ASC")
	...
	@Query("SELECT DISTINCT o FROM Order o " + "JOIN FETCH o.items oi " + "JOIN oi.product p "
			+ "LEFT JOIN FETCH p.ingredients ri " + "LEFT JOIN FETCH ri.inventoryItem ii "
			+ "WHERE o.status = 'DELIVERED' " + "AND (:start IS NULL OR o.orderDate >= :start) "
			+ "AND (:end IS NULL OR o.orderDate <= :end) " + "AND (:keyword IS NULL OR "
			+ "    CAST(o.id AS string) LIKE CONCAT('%', :keyword, '%') OR "
			+ "    LOWER(o.shippingFirstName) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
			+ "    LOWER(o.shippingLastName) LIKE LOWER(CONCAT('%', :keyword, '%'))" + ") "
			+ "ORDER BY o.orderDate DESC")
	List<Order> findDeliveredOrdersWithCogsDetails(@Param("keyword") String keyword,
